try:
    from chuscraper import cdp, loop, start
...
async def main() -> None:
    ...
    await search_inp.send_keys("undetected chuscraper")
    ...
    for letter in "undetected chuscraper":
        await search_inp.clear_input()
        await search_inp.send_keys(
            "undetected chuscraper".replace(letter, letter.upper())
        )
        await tab.wait(0.1)
